TweenMax.set(".seal-pattern", { opacity: 0, visibility: "visible" });
TweenMax.set('.typed-text span',{visibility:'visible'});
import { SplitText } from '/SplitText.js';
let typedText = new SplitText(".typed-text span", { type: "chars" });
...
// animate seals
function seals() {
let  sealTl = new TimelineLite();
  sealTl.staggerFromTo(
    ".seal-pattern",
    0.25,
...
/*function pictureLabelIn() {
  let tl  = new TimelineLite();
  tl.fromTo('#pro-pic',0.8,{opacity:0,rotation:-25,scale:1.5,x:-5,y:-75},{rotation:0,scale:1,opacity:1,x:0,y:0,ease: Back.easeInOut.config(1)},'in')
    .fromTo('.info-label',0.8,{opacity:0,rotation:25,scale:1.5,x:20,y:-50},{rotation:0,scale:1,opacity:1,x:0,y:0,ease: Back.easeInOut.config(1)},'in+=0.25')
  return tl;
}*/

let masterTl = new TimelineMax();
masterTl//.add( pictureLabelIn())
  .add(typeLetters(),0.5)
  .add(seals(), 3.9)
